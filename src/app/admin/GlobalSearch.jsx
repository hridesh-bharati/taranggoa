...
export default function GlobalSearch() {
  ...
  // Center popup open hote hi input bar focus ho
  useEffect(() => {
  ...
  return (
    <>
      ...
      {/* Center Screen Modal Backdrop (Instagram Style) */}
      {isOpen && (
        <div 
        ...
        >
          ...
          <div 
            ref={modalRef}
            className="bg-white rounded-4 shadow-lg border overflow-hidden w-100 d-flex flex-column" 
            style={{ maxWidth: '420px', maxHeight: '80vh' }}
          >
            {/* Input Header */}
            <div className="p-2.5 px-3 border-bottom d-flex align-items-center gap-2">
              {loading ? (
                <Loader2 size={15} className="spinner-border spinner-border-sm text-primary flex-shrink-0" />
              ) : (
              ...
              <input
                ref={inputRef}
                type="text"
                className="form-control border-0 bg-transparent py-1 fs-8 shadow-none p-0"
                placeholder="Search members, events, inbox..."
                value={query}
                onChange={(e) => setQuery(e.target.value)}
              />
              {query && (
                <button onClick={() => setQuery('')} className="btn btn-sm text-muted pe-1 py-0 border-0">
                  <X size={14} />
                </button>
              )}
              <button onClick={() => setIsOpen(false)} className="btn btn-sm text-muted p-0 border-0">
                <X size={16} />
              </button>
            </div>
            ...
            {/* Results Title Bar */}
            {query.length >= 2 && (
              <div className="p-2 px-3 border-bottom bg-light d-flex justify-content-between align-items-center">
            ...
            <div className="overflow-y-auto flex-grow-1">
              {totalCount === 0 && !loading && query.length >= 2 && (
